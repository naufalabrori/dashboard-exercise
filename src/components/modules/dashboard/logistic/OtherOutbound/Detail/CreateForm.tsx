...
import { useCreateOtherOutboundDetail } from "@/services/OtherOutbound/Detail/createOtherOutboundDetail";
import { z } from "zod";
...
export function CreateOtherOutboundDetailForm({
  headerId,
}: {
  headerId: string;
}) {
  ...
  const { mutate, isPending } = useCreateOtherOutboundDetail();
  ...
  const handleSubmit = () => {
    ...
    if (!result.success) {
      ...
      const payload = {
        otherOutboundHeaderId: headerId,
        ...
        remarkDoubleCheck: "",
        doubleCheckedBy: "",
        doubleCheckedOn: new Date().toISOString(),
      };

      mutate(payload, {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["get-other-outbound-detail"],
          });
          toast("Other Outbound Detail Created", { type: "success" });
        },
        onError: (error: any) => {
          toast(
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogTrigger asChild>
        ...
        >
          <PlusIcon />
          Create Other Outbound Detail
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-xl lg:max-w-[1000px]">
        <DialogHeader className="mb-2">
          <DialogTitle>Create Other Outbound Detail</DialogTitle>
        </DialogHeader>
