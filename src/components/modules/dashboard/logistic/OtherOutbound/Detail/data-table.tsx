...
import { OtherOutboundDetailColumns } from "./columns";
import { OtherOutboundDetail } from "@/services/OtherOutbound/Detail/types";
import { PaginationParams } from "@/lib/types";
import { useTablePagination } from "@/hooks/use-table-pagination";
import { useDebounce } from "use-debounce";
import { useListOtherOutboundDetail } from "@/services/OtherOutbound/Detail/getListOtherOutboundDetail";
...
function useTableData<TData>(
  ...
  const params: any = {
    ...
    otherOutboundHeaderId: otherInboundHeaderId,
  };
  ...
  const {
    ...
    error,
    isLoading,
  } = useListOtherOutboundDetail(params);
...
export function OtherOutboundDetailDataTable({
  headerId,
}: {
  headerId: string;
}) {
  ...
  const { data, total, loading, error } = useTableData<OtherOutboundDetail>(
    sorting,
    pagination,
    {
      key: filterBy,
  ...
  const queryKey = ["get-otherOutboundHeader-by-id", headerId];
  const cachedData: any = queryClient.getQueryData(queryKey);

  const columns: ColumnDef<any, OtherOutboundDetail>[] =
    OtherOutboundDetailColumns({
      currentPage,
